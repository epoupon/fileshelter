    ...
    : _processArg{ processArg }
{
    ...
    options.add_options()("conf,c", po::value<std::string>()->default_value("/etc/fileshelter.conf"), "fileshelter config file")("desc", po::value<std::string>()->default_value(""), "description")("password", po::value<std::string>()->default_value(""), "password")("validity-hours", po::value<unsigned>(), "validity period in hours")("validity-days", po::value<unsigned>(), "Validity period in days")("url,u", po::value<std::string>()->default_value(""), "deploy URL");

    po::options_description hiddenOptions{ "Hidden options" };
    hiddenOptions.add_options()("file", po::value<std::vector<std::string>>()->composing(), "file");

    _allOptions.add(options).add(hiddenOptions);
    _visibleOptions.add(options);
}

void CreateCommand::displayHelp(std::ostream& os) const
{
    os << "Usage: " << _processArg << " " << getName() << " [options] file...\n";
    os << _visibleOptions << std::endl;
}

int CreateCommand::process(const std::vector<std::string>& args) const
{
    namespace po = boost::program_options;
