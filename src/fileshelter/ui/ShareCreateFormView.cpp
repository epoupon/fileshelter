...
namespace UserInterface
{
    ...
    void ShareCreateFormView::deleteFile(Wt::WFileDropWidget::File& file)
    {
    ...
    void ShareCreateFormView::addFile(Wt::WFileDropWidget::File& file)
    {
        _totalSize += file.size();
        if (file.uploadFinished())
        {
    ...
    void ShareCreateFormView::onFileUploaded(const Wt::WFileDropWidget::File& file)
    {
    ...
    void ShareCreateFormView::visitUploadedFiles(std::function<void(Wt::WFileDropWidget::File& file)> visitor)
    {
    ...
    std::uint64_t ShareCreateFormView::getTotalFileSize() const
    {
    ...
    std::size_t ShareCreateFormView::countNbFilesToUpload() const
    {
        const auto& files{ _drop->uploads() };
    ...
    void ShareCreateFormView::checkFiles()
    {
    ...
    bool ShareCreateFormView::isUploadComplete() const
    {
        const auto& files{ _drop->uploads() };
    ...
    bool ShareCreateFormView::isFileDeleted(const Wt::WFileDropWidget::File& file) const
    {
        return _deletedFiles.find(&file) != std::cend(_deletedFiles);
    }

    void ShareCreateFormView::showError(const Wt::WString& error)
    {
        _error->setHidden(false);
        _error->bindString("error", error);
    }

    void ShareCreateFormView::hideError()
    {
        _error->setHidden(true);
    }

    std::filesystem::path ShareCreateFormView::getRelativeToWorkingDirectoryPath(const std::filesystem::path& path)
    {
        std::error_code ec;
