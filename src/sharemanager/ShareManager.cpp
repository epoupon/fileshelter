...
namespace ShareUtils
{
	...
	{
		std::optional<UUID> downloadUUID;
...

		{
			...
			downloadUUID = share->getDownloadUUID();

			for (const auto& file : share->getFiles())
			{
				if (file->isOwned())
					filesToRemove.push_back(file->getPath());
			}
		...
		for (const std::filesystem::path& file : filesToRemove)
		{
			std::error_code ec;
			std::filesystem::remove(file, ec);
			if (ec)
				FS_LOG(SHARE, ERROR) << "Cannot remove file: '" << file.string() << "' from share! Error:" << ec.message();
		}

		{
			...
			FS_LOG(SHARE, INFO) << "Destroyed share '" << downloadUUID->getAsString() << "'";
		}
	}
